...
#from scipy import stats，numpy中没有直接求众数的函数，stats中有此函数：stats.mode(nums)[0][0]
...
#二.数据集导入
print('--------------------数据导入--------------------\n')
os.chdir(r'F:\资料\信用卡项目最终代码\lending club(美国P2P放款业务中介公司) 贷款数据集') #设置工作路径
loan_data = pd.read_csv('loan_data.csv', encoding='utf-8')
'''
...
print(loan_data.info(verbose=True, null_counts=True)) #设置显示所有特征以及显示统计空值的参数
#1.是否显示所有列的信息，如果为否，那么会省略一部分
#2.null_counts：True or False，默认为True，是否统计NaN值的个数
...
loan_data['new_emp_length']=loan_data['emp_length'].apply(f)
loan_data.drop('emp_length', axis=1, inplace=True)#或者del loan_data["emp_length"]
loan_data.rename(columns={'new_emp_length':'emp_length'}, inplace = True)

#分组汇总，计算最大值，最小值，平均值
var=['loan_amnt', 'term', 'annual_inc', 'dti', 'delinq_2yrs', 'mths_since_last_delinq', 'open_acc', 'pub_rec', 'total_acc', 'acc_now_delinq',\
     'open_il_24m', 'inq_last_12m', 'num_accts_ever_120_pd', 'pct_tl_nvr_dlq', 'pub_rec_bankruptcies','emp_length']
grouped11=loan_data.groupby(['home_ownership'])[['loan_amnt','annual_inc','total_acc','dti']].agg([np.max,np.min,np.median,np.mean])
print("根据'home_ownership'分组计算'loan_amnt','annual_inc','total_acc','dti'结果:\n{}".format(grouped11))
print()
grouped12=loan_data.groupby(['home_ownership'])[['mths_since_last_delinq','pct_tl_nvr_dlq']].agg([np.max,np.min,np.median,np.mean])
print("根据'home_ownership'分组计算结果:\n{}".format(grouped12))
print()
grouped21=loan_data.groupby(['verification_status'])[['loan_amnt','annual_inc','total_acc','dti']].agg([np.max,np.min,np.median,np.mean])
print("根据'verification_status'分组计算指标'loan_amnt','annual_inc','total_acc','dti'结果:\n{}".format(grouped21))
print()
grouped22=loan_data.groupby(['verification_status'])[['mths_since_last_delinq','pct_tl_nvr_dlq']].agg([np.max,np.min,np.median,np.mean])
print("根据'verification_status'分组计算'mths_since_last_delinq','pct_tl_nvr_dlq'结果:\n{}".format(grouped22))
print()
grouped31=loan_data.groupby(['term'])[['loan_amnt','annual_inc','total_acc','dti']].agg([np.max,np.min,np.median,np.mean])
print("根据'term'分组计算'loan_amnt','annual_inc','total_acc','dti'结果:\n{}".format(grouped31))
print()
grouped32=loan_data.groupby(['term'])[['mths_since_last_delinq','pct_tl_nvr_dlq']].agg([np.max,np.min,np.median,np.mean])
print("根据'term'分组计算'mths_since_last_delinq','pct_tl_nvr_dlq'结果:\n{}".format(grouped32))
print()
grouped41=loan_data.groupby(['purpose'])[['loan_amnt','dti']].agg([np.max,np.min,np.mean])
print("根据'purpose'分组计算'loan_amnt','total_acc','dti'结果:\n{}".format(grouped41))
print()
grouped42=loan_data.groupby(['purpose'])[['mths_since_last_delinq']].agg([np.max,np.min])
...
print("分类变量的众数分别为：\n'home_ownship': {} \n'verification_status'：{}\n'term'：{} \n'purpose': {}".format(mode['home_ownership'][0],\
...
ax8 = plt.subplot2grid((4, 4), (3, 0), colspan=3)
...
inc_ver_target=loan_data.pivot_table(index='new_annual_inc', columns='verification_status', values='target', aggfunc='mean')
...
loan_data1 = loan_data.loc[(loan_data['home_ownership'] == 'MORTGAGE')|(loan_data['home_ownership']=='RENT') | (loan_data['home_ownership']=='OWN')]
home_ownership1_data = np.array(loan_data1.loc[loan_data['home_ownership'] == 'MORTGAGE','target'].count()/loan_data['target'].count())
home_ownership2_data = np.array(loan_data1.loc[loan_data['home_ownership'] == 'RENT','target'].count()/loan_data['target'].count())
home_ownership3_data = np.array(loan_data1.loc[loan_data['home_ownership'] == 'OWN','target'].count()/loan_data['target'].count())
...
patches, texts, autotexts = ax10.pie(home_ownership_data, labels=ax10_labels, autopct='%1.1f%%', explode=explode, colors = colors, radius=1)
#设置图内字体大小及颜色
...
#1.去除每一个特征中重复值高于百分之95的特征
def drop_duplicate(x):
	if ((loan_data[x].value_counts().sort_values(ascending=False).max()/loan_data[x].value_counts().sum())>0.95):
		print('去除的特征为{0}，该特征内重复值占比为{1:.4f}\n'.format(x,(loan_data[x].value_counts().sort_values(ascending=False).max()/loan_data[x].value_counts().sum())))
		del loan_data[x]
...
columns2=(loan_data.apply(lambda x: sum(x.isnull())/len(x),axis=0)>0.3)&(loan_data.apply(lambda x: sum(x.isnull())/len(x),axis=0)<0.6)
#print(columns2)
...
loan_data['mths_since_last_delinq_new'] =  pd.cut(loan_data['mths_since_last_delinq'], bins = bins,labels=labels,right=False) #新增一列年龄分段列
print(loan_data['mths_since_last_delinq_new'])
print(loan_data.groupby(by='mths_since_last_delinq_new')['target'].mean())'''

#缺失值比例小于30%，利用median或mean进行填充
columns3=(loan_data.apply(lambda x: sum(x.isnull())/len(x),axis=0)<0.3)&(loan_data.apply(lambda x: sum(x.isnull())/len(x),axis=0)>0)
#print(columns3)
...
#loan_data['target']=np.where(loan_data['target']=='Charged Off',1.0,0.0)
...
def Chi2(df, total, bad,overallRate):
    ...
    chi3=chi1+chi2
    return chi3
#可在定义一个函数检查样本种类是否齐全（是否0/1都有）以及样本是否满足分箱箱体最小值（该最小值应该基于原始样本量）
#分箱原则：最大分箱数分箱
def ChiMerge_MaxInterval_Original(df, col, max_interval=5):
    total=df.groupby([col])['target'].count()
    df_total=pd.DataFrame({'total':total})
    bad=df.groupby([col])['target'].sum()
    df_bad=pd.DataFrame({'bad':bad})
...
#type1:直接卡方分箱的连续性变量
L0=ChiMerge_MaxInterval_Original(loan_data, 'emp_length', max_interval=5)#L:tuple
L1=ChiMerge_MaxInterval_Original(loan_data, 'open_acc', max_interval=2)
L2=ChiMerge_MaxInterval_Original(loan_data, 'total_acc', max_interval=5)
L3=ChiMerge_MaxInterval_Original(loan_data, 'inq_last_12m', max_interval=4)
loan_data.loc[loan_data['mths_since_last_delinq']==999,'mths_since_last_delinq']=np.NAN
L4=ChiMerge_MaxInterval_Original(loan_data, 'mths_since_last_delinq', max_interval=4)
#L5=ChiMerge_MaxInterval_Original(loan_data, 'acc_rat', max_interval=5)


print("-----------------type1:bad rate 单调性检验---------------")
print("'{}':{}".format(L0[1].columns[0],BadRateMonotone(L0[1], L0[1].columns[0],L0[0])))
print("'{}':{}".format(L1[1].columns[0],BadRateMonotone(L1[1], L1[1].columns[0],L1[0])))
print("'{}':{}".format(L2[1].columns[0],BadRateMonotone(L2[1], L2[1].columns[0],L2[0])))
print("'{}':{}".format(L3[1].columns[0],BadRateMonotone(L3[1], L3[1].columns[0],L3[0])))
print("'{}':{}".format(L4[1].columns[0],BadRateMonotone(L4[1], L4[1].columns[0],L4[0])))
...
#type2:间接卡方分箱的连续性变量   
#考虑现将变量进行无监督等频分隔，再用有监督分隔作出权衡
#annual_inc
qcut1=pd.qcut(loan_data['annual_inc'], 10)
loan_data['inc_level']=loan_data['annual_inc']

loan_data.loc[(loan_data['annual_inc']>-0.001)&(loan_data['annual_inc']<=32000),'inc_level']=1
loan_data.loc[(loan_data['annual_inc']>32000)&(loan_data['annual_inc']<=40000),'inc_level']=2
loan_data.loc[(loan_data['annual_inc']>40000)&(loan_data['annual_inc']<=47250),'inc_level']=3
loan_data.loc[(loan_data['annual_inc']>47250)&(loan_data['annual_inc']<=54000),'inc_level']=4
loan_data.loc[(loan_data['annual_inc']>54000)&(loan_data['annual_inc']<=60000),'inc_level']=5
loan_data.loc[(loan_data['annual_inc']>60000)&(loan_data['annual_inc']<=70000),'inc_level']=6
loan_data.loc[(loan_data['annual_inc']>70000)&(loan_data['annual_inc']<=80000),'inc_level']=7
loan_data.loc[(loan_data['annual_inc']>80000)&(loan_data['annual_inc']<=90000),'inc_level']=8
loan_data.loc[(loan_data['annual_inc']>90000)&(loan_data['annual_inc']<=110000),'inc_level']=9
loan_data.loc[(loan_data['annual_inc']>110000)&(loan_data['annual_inc']<=161000),'inc_level']=10
...
loan_data.loc[(loan_data['dti']>-0.001) & (loan_data['dti']<=5.1),'dti_type']=1
loan_data.loc[(loan_data['dti']>5.1) & (loan_data['dti']<=7.7),'dti_type']=2
loan_data.loc[(loan_data['dti']>7.7) & (loan_data['dti']<=9.76),'dti_type']=3
loan_data.loc[(loan_data['dti']>9.76) & (loan_data['dti']<=11.5),'dti_type']=4
loan_data.loc[(loan_data['dti']>11.5) & (loan_data['dti']<=13.08),'dti_type']=5
loan_data.loc[(loan_data['dti']>13.08) & (loan_data['dti']<=14.57),'dti_type']=6
loan_data.loc[(loan_data['dti']>14.57) & (loan_data['dti']<=16.02),'dti_type']=7
loan_data.loc[(loan_data['dti']>16.02) & (loan_data['dti']<=17.53),'dti_type']=8
loan_data.loc[(loan_data['dti']>17.53) & (loan_data['dti']<=19.16),'dti_type']=9
loan_data.loc[(loan_data['dti']>19.16) & (loan_data['dti']<=20.86),'dti_type']=10
loan_data.loc[(loan_data['dti']>20.86) & (loan_data['dti']<=22.7),'dti_type']=11
loan_data.loc[(loan_data['dti']>22.7) & (loan_data['dti']<=24.83),'dti_type']=12
loan_data.loc[(loan_data['dti']>24.83) & (loan_data['dti']<=27.83),'dti_type']=13
loan_data.loc[(loan_data['dti']>27.83) & (loan_data['dti']<=30.6),'dti_type']=14
loan_data.loc[(loan_data['dti']>30.6) & (loan_data['dti']<=42.6),'dti_type']=15
...
loan_data.loc[(loan_data['loan_amnt']>999.999)&(loan_data['loan_amnt']<=3200),'amnt_level']=1
loan_data.loc[(loan_data['loan_amnt']>3200)&(loan_data['loan_amnt']<=5000),'amnt_level']=2
loan_data.loc[(loan_data['loan_amnt']>5000)&(loan_data['loan_amnt']<=6000),'amnt_level']=3
loan_data.loc[(loan_data['loan_amnt']>6000)&(loan_data['loan_amnt']<=8000),'amnt_level']=4
loan_data.loc[(loan_data['loan_amnt']>8000)&(loan_data['loan_amnt']<=9600),'amnt_level']=5
loan_data.loc[(loan_data['loan_amnt']>9600)&(loan_data['loan_amnt']<=10590),'amnt_level']=6
loan_data.loc[(loan_data['loan_amnt']>10590)&(loan_data['loan_amnt']<=14000.0),'amnt_level']=7
loan_data.loc[(loan_data['loan_amnt']>14000)&(loan_data['loan_amnt']<=17000),'amnt_level']=8
loan_data.loc[(loan_data['loan_amnt']>17000)&(loan_data['loan_amnt']<=24000),'amnt_level']=9
loan_data.loc[(loan_data['loan_amnt']>24000)&(loan_data['loan_amnt']<=39600),'amnt_level']=10
...
#pd.value_counts(qcut2,ascending=True)
loan_data.loc[(loan_data['acc_rate']>-0.001) & (loan_data['acc_rate']<=0.273),'acc_type']=1
loan_data.loc[(loan_data['acc_rate']>0.273) & (loan_data['acc_rate']<=0.333),'acc_type']=2
loan_data.loc[(loan_data['acc_rate']>0.333) & (loan_data['acc_rate']<=0.367),'acc_type']=3
loan_data.loc[(loan_data['acc_rate']>0.367) & (loan_data['acc_rate']<=0.4),'acc_type']=4
loan_data.loc[(loan_data['acc_rate']>0.4) & (loan_data['acc_rate']<=0.436),'acc_type']=5
loan_data.loc[(loan_data['acc_rate']>0.436) & (loan_data['acc_rate']<=0.467),'acc_type']=6
loan_data.loc[(loan_data['acc_rate']>0.467) & (loan_data['acc_rate']<=0.5),'acc_type']=7
loan_data.loc[(loan_data['acc_rate']>0.5) & (loan_data['acc_rate']<=0.529),'acc_type']=8
loan_data.loc[(loan_data['acc_rate']>0.529) & (loan_data['acc_rate']<=0.562),'acc_type']=9
loan_data.loc[(loan_data['acc_rate']>0.562) & (loan_data['acc_rate']<=0.6),'acc_type']=10
loan_data.loc[(loan_data['acc_rate']>0.6) & (loan_data['acc_rate']<=0.636),'acc_type']=11
loan_data.loc[(loan_data['acc_rate']>0.636) & (loan_data['acc_rate']<=0.688),'acc_type']=12
loan_data.loc[(loan_data['acc_rate']>0.688) & (loan_data['acc_rate']<=0.75),'acc_type']=13
loan_data.loc[(loan_data['acc_rate']>0.75) & (loan_data['acc_rate']<=0.833),'acc_type']=14
loan_data.loc[(loan_data['acc_rate']>0.833) & (loan_data['acc_rate']<=1.0),'acc_type']=15

acc_type = loan_data.pop('acc_type')
loan_data.insert(19,'acc_type',acc_type)

K0=ChiMerge_MaxInterval_Original(loan_data, 'inc_level', max_interval=5)
K1=ChiMerge_MaxInterval_Original(loan_data, 'dti_type', max_interval=5)
K2=ChiMerge_MaxInterval_Original(loan_data, 'amnt_level', max_interval=5)
K3=ChiMerge_MaxInterval_Original(loan_data, 'acc_type', max_interval=4)

print("-----------------type2:bad rate 单调性检验---------------")
print("{}:{}".format(K0[1].columns[0],BadRateMonotone(K0[1], K0[1].columns[0],K0[0])))
print("{}:{}".format(K1[1].columns[0],BadRateMonotone(K1[1], K1[1].columns[0],K1[0])))
print("{}:{}".format(K2[1].columns[0],BadRateMonotone(K2[1], K2[1].columns[0],K2[0])))
print("{}:{}".format(K3[1].columns[0],BadRateMonotone(K3[1], K3[1].columns[0],K3[0])))
print('检验通过')
...
M0=ChiMerge_MaxInterval_Original(loan_data, 'purpose', max_interval=5)
M1=ChiMerge_MaxInterval_Original(loan_data, 'addr_state', max_interval=5)

print("-----------------type3:bad rate 单调性检验---------------")
print("{}:{}".format(M0[1].columns[0],BadRateMonotone(M0[1], M0[1].columns[0],M0[0])))
print("{}:{}".format(M1[1].columns[0],BadRateMonotone(M1[1], M1[1].columns[0],M1[0])))
print('检验通过')
...
v0=['0~2 year','3 year','4~5 year','6 year','7~10 year']
v1=['0-3个','4-24个']
v2=['2-4个','5-8个','9-19个','20-34个','35-52个']
v3=['0.0次','1-5次','6次','7次']
v4=['0~21 mths','22~49 mths','50~69 mths','70~100 mths','999']

#v5=['level 1(1)','level 2(2)','level 3(3-8)','level 4(9)','level 5(10)']
v5=['level 1(-0.0001-32000)','level 2(32000-40000)','level 3(40000-90000)','level 4(90000-110000)','level 5(110000-161000)']
#v6=['level 1(1-3)','level 2(4-6)',' level 3(7-13)','level 4(14)','level 5(15)']
v6=['level 1(-0.001-9.76)','level 2(9.76-14.57)',' level 3(14.57-27.83)','level 4(27.83-30.6)','level 5(30.6-42.6)']
#v7=['level 1(1)','level 2(2-3)','level 3(4-8)','level 4(9)','level 5(10)']
v7=['level 1(999.9-3200)','level 2(3200-6000)','level 3(6000-17000)','level 4(17000-24000)','level 5(24000-396000)']
#v8=['level 1(1-2)','level 2(3-12)','level 3(13)','level 4(14)','level 5(15)']
v8=['level 1(-0.001-0.333)','level 2(0.333-0.688)','level 3(0.688-0.75)','level 4(0.75-0.833)','level 5(0.833-1.0)']

v9=['level 1(0.11-0.1394)','level 2(0.1394-0.1399)','level 3(0.1399-0.171)','level 4(0.171-0.225)','level 5(0.225-0.286)']
v9lab=["('car', 'home_improvement')",'credit_card',"('debt_consolidation', 'house', 'major_purchase', 'other', 'vacation')","('medical', 'moving', 'renewable_energy')","'small_business'"]
#(set(p1),set(p2),set(p3),set(p4),set(p5))

v10=['level 1(0.0-0.103)','level 2(0.103-0.112','level 3(0.112-0.122)','level 4(0.122-0.178)','level 5(0.178-0.224)']
v10lab=["{'DC', 'ME', 'RI', 'UT', 'VT', 'WV'}","{'CO', 'OR'}","{'AZ', 'GA', 'SC', 'WA'}","{'AK', 'AL', 'AR', 'CA', 'CT', 'FL', 'HI', 'ID', 'IL', 'IN', 'KS', 'KY', 'LA', 'MA', 'MD', 'MI', 'MN','NC','NH', 'NJ', 'NM', 'NV','OH','PA','TN', 'TX','VA','WI','WY'}","{'DE', 'MO', 'MS', 'MT', 'NE', 'NY', 'OK', 'SD'}"]
#(set(field1),set(field2),set(field3),set(field4),set(field5))

V=[v0,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10]
for L,v in zip(L_type1,V):
    for interval,value in zip(L[0],v):
        #print(interval)
        #print(interval,value)
        datachi2.loc[datachi2[L[1].columns[0]].isin(interval),L[1].columns[0]]=value
...
datachi2.rename(columns={'amnt_level': 'loan_amnt', 'inc_level': 'annual_inc','dti_type': 'dti','acc_type':'acc_rat'}, inplace=True)
#datachi2.drop(['addr_state_rate','purpose_rate'],axis=1,inplace=True)

#3、类别数<5或取值较少且偏态分布严重的变量的分箱结果以及分箱后bad rate单调性检测
#(过去2年中逾期30天以上违约事件的次数)分箱
bins_delinq_2yrs=[0,1,2,100]
labels_delinq_2yrs=['0次','1次','>2次']
datachi2['delinq_2yrs'] =  pd.cut(datachi2['delinq_2yrs'], bins = bins_delinq_2yrs,labels=labels_delinq_2yrs,right=False) #delinq_2yrs列分段
...
datachi2['open_il_24m'] =  pd.cut(datachi2['open_il_24m'], bins = bins_open_il_24m,labels=labels_open_il_24m,right=False) #open_il_24m列分段
...
datachi2['num_accts_ever_120_pd'] =  pd.cut(datachi2['num_accts_ever_120_pd'], bins = bins_num_accts_ever_120_pd,labels=labels_num_accts_ever_120_pd,right=False) #num_accts_ever_120_pd列分段
...
datachi2['pct_tl_nvr_dlq'] =  pd.cut(datachi2['pct_tl_nvr_dlq'], bins = bins_pct_tl_nvr_dlq,labels=labels_pct_tl_nvr_dlq,right=False) #pct_tl_nvr_dlq列分段
...
datachi2['pub_rec_bankruptcies'] =  pd.cut(datachi2['pub_rec_bankruptcies'], bins = bins_pub_rec_bankruptcies,labels=labels_pub_rec_bankruptcies,right=False) #delinq_2yrs列分段
#print(datachi2.groupby('pub_rec_bankruptcies')['target'].mean())

#最终的数据集dataframe:datachi2
#构建输出含义表
essence_df=pd.DataFrame({'pur_field':v9,'pur_essence':v9lab,'addr_field':v10,'addr_essence':v10lab})
print(essence_df)
...
datachi2=datachi2.loc[(datachi2['home_ownership']=='MORTGAGE')|(datachi2['home_ownership']=='RENT')|(datachi2['home_ownership']=='OWN')]
...
for i in var: #i在所有的特征中遍历
	...
	for j in range(len(datachi2[i].unique())): #j在每一个特征的分箱中进行遍历
		...
		good_rat=(datachi2.groupby(i)['target'].count().iloc[j]-datachi2.groupby(i)['target'].sum().iloc[j])/total_good_num
		bad_rat=datachi2.groupby(i)['target'].sum().iloc[j]/total_bad_num
		Woe=float(np.log((bad_rat)/(good_rat))) 
		IV=(bad_rat-good_rat)*Woe
		IV_total+=IV
		Woe_list.append(Woe)
	...
	datachi2[i]=datachi2[i].map(var_mapping)
print(var_IV)
print(datachi2.head(10))
print('\nWoe编码完成')
...
dict_IV_sort=sorted(dict_IV.items(),key=lambda x:x[1],reverse=True) #返回的是一个嵌套列表
#生成数据集和标签并画图
keys=[]
values=[]
...
plt.hlines(0.02,18.1,-0.5,linestyle = 'dashed')
plt.text(18.15,0.019,'0.02',fontsize=11)
...
#选取IV值>0.02的变量
retain_var=[]
for i,j in zip(dict_IV.keys(),dict_IV.values()):
	if j>=0.02:
		retain_var.append(i)
...
random_good_index=np.random.choice(good_index,bad_num,replace=True)
...
final_dataset.to_csv(r'C:\Users\CX\Desktop\lending club(美国P2P放款业务中介公司) 贷款数据集\final_dataset.csv',index=False) #最终数据集(下采样后)
assist_dataset.to_csv(r'C:\Users\CX\Desktop\lending club(美国P2P放款业务中介公司) 贷款数据集\assist_dataset.csv',index=False) #最终辅助数据集(下采样后)
print('\n下采样完成')
